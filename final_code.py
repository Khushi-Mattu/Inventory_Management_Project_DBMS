...
class Login_Page:
    ...
    def __init__(self, login=Tk()):
        ...
        login.title("Electrical Inventory")
    ...
    # Creating describtioneves
        ...
        self.username_box.place(relx=0.440, rely=0.298,
                                height=20, relwidth=0.35)

        self.password_box = Entry(login)
        self.password_box.place(relx=0.440, rely=0.468,
                                height=20, relwidth=0.35)
        self.password_box.configure(show="*")
        self.password_box.configure(background="black")

        # Creating checkbox

        self.var = IntVar()
        self.show_password = Checkbutton(login)
        self.show_password.place(
            relx=0.285, rely=0.650, relheight=0.100, relwidth=0.125)
    ...
    def login_user(self):
        ...
        if name == "user" and password == "1234":
            ...
            messagebox.showwarning(
                "Login Failed - Acess Denied", "Username or Password incorrect!")

        # return

    def exit_login(self):
        msg = messagebox.askyesno(
            "Exit login page", "Do you really want to exit?")
        if (msg):
            exit()

    # This is the class function that helps me to mainloop the window
    def mainloop_window(self):
        self.login.mainloop()


# I dont need to pass the root now since its initialized inside the class
            ...
            if records == []:
                ...
                for record in records:
                    print_records += str(record[0]) + " " + str(record[1]) + " " + str(record[2]) + " " + str(
                        record[3]) + " " + str(record[4]) + " " + str(record[5]) + " " + str(record[6]) + " " + str(record[7]) + "\n"
...
database= Main_Win()  # I dont need to pass the root now since its initialized inside the class
database.mainloop_window()  # Just mainlooping the authentication window
